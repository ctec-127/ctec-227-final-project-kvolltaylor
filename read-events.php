        ...
        <div class='col-sm-9 center'>
            ...
            <div class="col-12 wh-bkgd pt-2 pb-4 mb-4">
                ...
                <?php
                    ...
                    // filtering by tag if tag is clicked
                    if (isset($_GET['tag'])) {
                        $get_tag = $_GET['tag'];
                    ...
                    // filtering by entry id if Anxiety 'Yes' is clicked on entry page
                    } elseif (isset($_GET['entry'])) {
                        $get_entry = $_GET['entry'];
                        $sql_event = "SELECT * FROM `event`  
                                      WHERE entry_id='$get_entry' 
                                      ORDER BY `event_id` 
                                      DESC";
                        unset($get_event);

                        echo "<div class='h5 text-center text-secondary'><a href='read-entries.php'>>Return to reading entries<</a></div>";

                    // default behavior, show all for user
                    } else {
                        $sql_event = "SELECT * FROM `event` 
                                WHERE user_id='$user_id' 
                                ORDER BY `event_id` 
                    ...
                    while ($row = $result_event->fetch_assoc()) {
                        ...
                        echo "<a href='inc/content/delete/delete-event.inc.php?id={$row['event_id']}' onclick='return confirm(\"Are you sure you want to delete this?\");' class='btn custom-delete ml-4 mb-4'>Delete</a>";
